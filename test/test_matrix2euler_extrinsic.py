...
def test_all_valid_axes():
    ...
    for axes in valid_axes:
        rotation_matrix = euler2matrix(eulers, axes=axes, intrinsic=False, positive_ccw=True)

        result_eulers = matrix2euler(rotation_matrix, axes=axes, positive_ccw=True,
                              intrinsic=False)
        result_rotation_matrix = euler2matrix(result_eulers, axes=axes, intrinsic=False,
                                       positive_ccw=True)
